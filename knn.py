...
class KNN:
    ...
    def get_k_neighbors(self, point):
        ...
        for i in range(len_f):
            ...
            n_dis.append((dist,self.labels[i]))
        n_dis.sort(key=lambda x:x[0])
        for i in range(self.k):
            knn_label.append(n_dis[i][1])
        return(knn_label)
    ...
    def predict(self, features):
        ...
        len_f=len(features)
        for i in features:
            label=self.get_k_neighbors(i)
            train.append(Counter(label).most_common(1)[0][0])
        return(train)
