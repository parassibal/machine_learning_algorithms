...
class linear_layer:
    ...
    def forward(self, X):
        ...
        ################################################################################
        X_w=np.dot(X,self.params['W'])
        forward_output=X_w+self.params['b']
...
class relu:
    ...
    def forward(self, X):
        ...
        size=np.size(X,0)
        size1=np.size(X,1)
        forward_output=np.maximum(X,np.zeros(size,size1))
        forward_output=np.copy(X)
...
class tanh:
    ...
    def backward(self, X, grad):
        ...
        ####################################################################################################
        der=1-np.power(np.tanh(X),2)
        backward_output=grad*der
        return(backward_output)
...
class dropout:
    ...
    def backward(self, X, grad):
        ...
        # You can use the mask created in the forward step
        ####################################################################################################
        backward_output=grad*self.masks
        return(backward_output)
